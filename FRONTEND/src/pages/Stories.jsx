...
const Catalog = () => {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        ...
        if(!allPostRes.data.data) throw new Error("stories fetching error or not logged in")
  ...
  // const handleForwardClick = async function(){
  //   try {
  //     const linkData = await axios.get("posts/")
  //   } catch (error) {
  ...
  <div className="h-screen ">
    ...
    <div className="container mx-auto px-8 py-6 flex flex-col sm:flex-row items-center sm:items-end justify-between gap-4">
      {/* Search Bar */}
    ...
    {/* Cards */}
    <div className="container mx-auto px-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-80 h-[1200px] mt-10">
      {fetchedData.map((data, index) => (
            <Link
              to={"/viewPage/"+data._id}  // Link to the specified route
              key={data._id}
              className={`relative bg-white rounded-3xl overflow-hidden shadow-md mx-auto w-full max-w-xs lg:max-w-md opacity-0 transform translate-y-10 transition-all duration-700 ease-in-out ${
