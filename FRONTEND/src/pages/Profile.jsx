...
function Profile() {
  ...
  useEffect(() => {
    ...
    async function updateData() {
      try {
        const response = await axios.get("https://historia-2-0-1.onrender.com/users/getUserProfile",{
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await axios.get("https://historia-2-0-1.onrender.com/posts/getUserPosts",{
          withCredentials:true
        });
  ...
  const logOutMethod = async () => {
    try {
      const logoutStatus = await axios.get("https://historia-2-0-1.onrender.com/users/logout",{
        withCredentials:true
      });
      if (!logoutStatus) throw new Error("https://historia-2-0-1.onrender.com/Logout failure");
      dispatcher(logout());
      navigate("/");
    } catch (error) {
      console.error(error);
  ...
  async function deletePost(e, data) {
    try {
      const deletedPost = await axios.get(`https://historia-2-0-1.onrender.com/posts/deletePost/${data}`);
